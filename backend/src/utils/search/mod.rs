...
use crate::utils::events::{map_events, EventQuery, QOverride, RecurrenceJSON};
...
impl<'c> PgQuery<'c, Search> {
    ...
    pub async fn get_owned_events(
        ...
        let events = query!(
            r#"
                SELECT id, name, description, starts_at, COALESCE(until, ends_at) AS entries_end, recurrence AS "recurrence: Option<sqlx::types::Json<RecurrenceJSON>>", until
                FROM events
    ...
    pub async fn get_shared_events(
        ...
        let events = query!(
            r#"
                SELECT id, name, description, starts_at, COALESCE(until, ends_at) AS entries_end, recurrence AS "recurrence: Option<sqlx::types::Json<RecurrenceJSON>>", can_edit, until 
                FROM user_events
